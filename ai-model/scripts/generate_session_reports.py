...
def generate_report_for_folder(folder: Path) -> bool:
    ...
    if gesture_counts:
        for gname, count in sorted(gesture_counts.items(), key=lambda x: x[1], reverse=True):
            ...
            quality = "\U0001f7e2 High Frequency" if pct >= 20 else "\U0001f535 Standard"
    ...
    report_lines.extend([
        ...
        "| :--- | :---: | :---: | :---: |",
        f"| **Camera Frame Acquisition** | ~{avg_fps} FPS | $\ge 28$ FPS | {fps_status} |",
...
def main() -> None:
    ...
    for entry in sorted(RECORDINGS_DIR.iterdir()):
        if entry.is_dir() and (entry / "summary.json").exists():
            if generate_report_for_folder(entry):
